...
def index():
    ...
    return render_template('indextest.html', usermsg=g.user, picid=str(g.user.picid), picids=picids, **context)
...
def uploadpic():
    if request.method == 'POST':
        ...
        if file:
            # picway = '/home/ubuntu/HITChat/static/images/userimage/' + str(g.user.telephone) + str(g.user.picid) + '.png' #服务器
            picway = os.getcwd() + 'static/images/userimage/' + str(g.user.telephone) + str(g.user.picid) + '.png' #本机
            ...
            db.session.add(usermsg)
            db.session.commit()
            # file.save(os.path.join('/home/ubuntu/HITChat', 'static/images/userimage/' + str(g.user.telephone) + str(g.user.picid) + '.png')) #服务器
            file.save(os.path.join(os.getcwd(), 'static/images/userimage/' + str(g.user.telephone) + str(g.user.picid) + '.png')) #本机
...
def jwcdetail(news_id):
    try:
        sql = mysql()
        datajwc = sql.selectnews('jwcArticle',news_id) 
        #print datajwc
        sql.conn.close()
