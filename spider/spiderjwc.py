...
import uuid
from sendmessage import sendmsmessage
...
def spiderArtitle(num,Sum):
    ...
    for i in range(len(s)):
        ...
        if (Sum == 1 and i == 0 and len(data)>1 and articleurl == data[0][2]):
            telephone = 18804652328
            alycode = sendmsmessage()
            __business_id = uuid.uuid1()
            #print __business_id
            yzcode = str(18804652328)
            params = "{\"code\":"+yzcode+",\"product\":\"云通信\"}"
            print alycode.send_sms(__business_id, telephone, "HITChat项目组", "SMS_116560786",params)
...
data = sql.selectfirst("jwcarticle")
print data
